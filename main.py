...
def yellow_moves(keys_pressed, yellow):
    ...
    if keys_pressed[pygame.K_w] and yellow.y > 0:  # UP
        yellow.y -= VEL
    if keys_pressed[pygame.K_s] and yellow.y < HEIGHT - yellow.height:  # DOWN
        yellow.y += VEL


def red_moves(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x > BORDER.x:  # LEFT
        red.x -= VEL
    if keys_pressed[pygame.K_RIGHT] and red.x < WIDTH - red.width:  # RIGHT
        red.x += VEL
    if keys_pressed[pygame.K_UP] and red.y > 0:  # UP
        red.y -= VEL
    if keys_pressed[pygame.K_DOWN] and red.y < HEIGHT - red.height:  # DOWN
        red.y += VEL
